#!/usr/bin/env python3
"""Protocol-1 comparison plots for 3DoF:

For each policy, create a separate figure with boxplots across lengths (50..1000),
comparing 5 methods:
  1) Unguided DPF
  2) Guided DPF
  3) Unguided Diffusion (fixed-length model, direct length-L generation)
  4) Guided Diffusion (fixed-length model, direct length-L generation)
...
OUT_ROOT = _env_path("OUT_ROOT", project_root / "output_ablation" / "protocol1_3dof_fixed_diffusion_eval_with_hnn")
...
DPF_NRMSE_CSV = _env_path(
    ...
    project_root
    / "output_ablation"
    / "default_full_sweep_3dof_latest_hnn999"
    / "metrics"
...
def run_diffusion_eval_if_needed():
    ...
    if len(done) >= expected_combos and DIFF_PER_SAMPLE_CSV.exists():
        print(f"[Skip] diffusion eval already complete ({len(done)}/{expected_combos}).")
        return
...
def load_dpf_table():
    ...
    ndf = ndf[(ndf["system"] == "3dof") & (ndf["length"] <= 1000)].copy()
    hdf = hdf[(hdf["system"] == "3dof") & (hdf["length"] <= 1000)].copy()
...
def load_diffusion_table():
    ...
    df = df[(df["system"] == "3dof") & (df["length"] <= 1000)].copy()
    return df
...
def make_policy_boxplots():
    ...
    for policy in POLICIES:
        ...
        for ax, (mkey, mlabel) in zip(axes, metrics):
            ...
            ax.set_title(f"3DoF | torque policy={policy}")
        ...
        out = PLOTS_ROOT / f"boxplot_3dof_{policy}_dpf_vs_diffusion_protocol1.png"
