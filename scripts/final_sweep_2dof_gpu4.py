...
def run_config(dpf, hnn, cfg_sys, mj_model, var_dq, var_dp, hamres_kwargs, qpos_dim, conf, settings, seeds, num_samples):
    rows = []
    for policy, L in settings:
        for seed in seeds:
            ...
            u_nq, u_np, u_hr, *_ = qe.compute_metrics_for_samples(
                ...
                var_dq, var_dp, qpos_dim, desc=f"ung_{policy}_L{L}_s{seed}", hamres_kwargs=hamres_kwargs
            )
            ...
            gui_states, gui_torques = qe.run_batch(
                ...
                guidance_steps=conf['steps'],
                guidance_lr=0.01,
                guidance_after_steps=conf['after'],
                guidance_before_steps=20,
                optimize_target='both',
                guidance_energy_mode=conf['mode'],
                guidance_hamres_smooth_sigma=conf.get('ham_sigma', 1.0),
                guidance_hamres_delta=conf.get('ham_delta', 1.0),
                guidance_hamres_min_scale_q=1e-3,
                guidance_hamres_min_scale_p=1e-3,
                guidance_trust_lambda=conf.get('trust', 0.0),
                smooth_sigma=0,
            ...
            g_nq, g_np, g_hr, *_ = qe.compute_metrics_for_samples(
                ...
                var_dq, var_dp, qpos_dim, desc=f"gui_{conf['name']}_{policy}_L{L}_s{seed}", hamres_kwargs=hamres_kwargs
            )
            ...
            rows.append(dict(
                ...
                ung_nq=u[0], ung_np=u[1], ung_hr=u[2],
                gui_nq=g[0], gui_np=g[1], gui_hr=g[2],
...
def summarize(rows):
    ...
    for name, rs in by.items():
        dnq = np.array([x['d_nq'] for x in rs])
        dnp = np.array([x['d_np'] for x in rs])
        dhr = np.array([x['d_hr'] for x in rs])
        ...
        # weighted score: prioritize all metrics but weight NMSE slightly higher
        score = 0.4 * float(np.median(dnq)) + 0.4 * float(np.median(dnp)) + 0.2 * float(np.median(dhr))
        out.append(dict(
            config=name,
            n=len(rs),
            all_win=f"{wall}/{len(rs)}",
            win_nq=f"{wnq}/{len(rs)}",
            win_np=f"{wnp}/{len(rs)}",
            win_hr=f"{whr}/{len(rs)}",
            med_d_nq=float(np.median(dnq)),
            med_d_np=float(np.median(dnp)),
            med_d_hr=float(np.median(dhr)),
            score=score,
        ))
...
if __name__ == '__main__':
    ...
    settings = [(p, L) for p in ['sinusoidal', 'gp', 'zero', 'spline'] for L in [100, 300, 1000]]
    seeds = [65, 66, 67]
    num_samples = 8
    ...
    for c in configs:
        ...
        all_rows.extend(run_config(dpf, hnn, cfg_sys, mj_model, var_dq, var_dp, hamres_kwargs, qpos_dim, c, settings, seeds, num_samples))

    summary = summarize(all_rows)

    out_dir = '/home/gsang/Projects/Perceiver_IO/output_ablation/sweep_outputs'
    os.makedirs(out_dir, exist_ok=True)
    rows_csv = os.path.join(out_dir, 'final_sweep_gpu4_rows.csv')
    sum_csv = os.path.join(out_dir, 'final_sweep_gpu4_summary.csv')
    ...
    for s in summary:
        print(s)
    print('rows_csv=', rows_csv)
    print('sum_csv=', sum_csv)
