#!/usr/bin/env python3
"""Protocol-1 comparison plots for 2DoF (DPF + HNN only).

For each torque policy, create one figure with boxplots across lengths (50..1000),
comparing 3 methods:
  1) Unguided DPF
  2) Guided DPF
  3) HNN Rollout (free rollout from unguided x0)
...
OUT_ROOT = project_root / "output_ablation" / "protocol1_2dof_dpf_hnn_eval"
METRICS_ROOT = OUT_ROOT / "metrics"
PLOTS_ROOT = project_root / "plots"
METRICS_ROOT.mkdir(parents=True, exist_ok=True)
PLOTS_ROOT.mkdir(parents=True, exist_ok=True)

PER_SAMPLE_CSV = METRICS_ROOT / "metrics_per_sample_nrmse_hamres.csv"
DONE_JSON = OUT_ROOT / "completed_combos.json"
...
def run_hnn_rollout_batch(hnn, init_states, torques):
    ...
    Returns:
        hnn_states: [N, T, D]
    """
    qdim = init_states.shape[-1] // 2
    q0 = init_states[:, 0, :qdim]
    p0 = init_states[:, 0, qdim:]
...
def compute_nrmse_range_and_hamres(state, tau, qpos_dim, mj_model, hnn, var_dq, var_dp, hamres_kwargs):
...
def run_eval_if_needed():
    expected_combos = len(POLICIES) * len(LENGTHS)
    done = set()
    if DONE_JSON.exists():
        done = set(json.loads(DONE_JSON.read_text()))

    if len(done) >= expected_combos and PER_SAMPLE_CSV.exists():
        print(f"[Skip] eval already complete ({len(done)}/{expected_combos}).")
        return
    ...
    print(f"[Eval] dpf checkpoint={cfg['dpf_ckpt']}")
    ...
    manifest = {
        ...
        "protocol": "protocol1_direct_length_L_generation",
    }
    ...
    for policy in POLICIES:
        for length in LENGTHS:
            ...
            hnn_states = run_hnn_rollout_batch(hnn, ung_states, torques)
...
def load_eval_table():
    if not PER_SAMPLE_CSV.exists():
        raise FileNotFoundError(f"Missing per-sample file: {PER_SAMPLE_CSV}")
    df = pd.read_csv(PER_SAMPLE_CSV)
    return df[(df["system"] == SYSTEM) & (df["length"] <= 1000)].copy()


def make_policy_boxplots():
    df = load_eval_table()
    lengths = sorted([l for l in df["length"].unique().tolist() if l <= 1000])
    ...
    for policy in POLICIES:
            ...
            for (method_name, color), offset in zip(methods, offsets):
                ...
                for L in lengths:
                    if method_name == "unguided_dpf":
                        arr = sub[sub["length"] == L][f"ung_{mkey}"].to_numpy()
                    elif method_name == "guided_dpf":
                        arr = sub[sub["length"] == L][f"gui_{mkey}"].to_numpy()
                    elif method_name == "hnn_rollout":
                        arr = sub[sub["length"] == L][f"hnn_{mkey}"].to_numpy()
                    else:
        ...
        axes[-1].set_xticklabels([str(L) for L in lengths], rotation=45, ha="right")
        ...
        out = PLOTS_ROOT / f"boxplot_2dof_{policy}_dpf_hnn_protocol1.png"
