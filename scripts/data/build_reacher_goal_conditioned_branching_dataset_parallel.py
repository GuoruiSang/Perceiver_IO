...
from scripts.data.generate_reacher_goal_conditioned_branching_dataset import generate_split, summarize_h5
...
def _copy_group_with_offsets(
    ...
    if "target_group_id" in dst_group.attrs:
        dst_group.attrs["target_group_id"] = int(dst_group.attrs["target_group_id"]) + target_offset
        ...
        for shard in tqdm(shard_results, desc="Merging shards"):
            ...
            with h5py.File(shard_path, "r") as shard_file:
                for traj_idx in range(num_traj):
                    ...
                    _copy_group_with_offsets(src_group, dst_group, source_offset, root_offset, anchor_offset, target_offset)
                    next_idx += 1
            source_offset += int(shard["generation"]["source_groups"])
            root_offset += int(shard["generation"]["root_groups"])
            anchor_offset += int(shard["generation"]["anchor_groups"])
            target_offset += int(shard["generation"]["target_groups"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build a large goal-conditioned branching Reacher dataset in parallel shards")
    ...
    parser.add_argument("--num_sources", type=int, default=1000)
    ...
    parser.add_argument(
        ...
        type=float,
        nargs="+",
        default=[0.000, 0.005, 0.010, 0.015, 0.020, 0.030, 0.040, 0.050, 0.065, 0.080, 0.100, 0.125, 0.150, 0.180, 0.210],
    )
    ...
    parser.add_argument("--momentum_bin_edges", type=float, nargs="+", default=[0.00, 0.02, 0.05, 0.10, 0.20, 0.40, 0.80, 1.60, 3.20])
