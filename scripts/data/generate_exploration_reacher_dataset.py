...
_WORKER_DT = None
_WORKER_IDS = None
_WORKER_TORQUE_LIMIT = None


def sample_xy_in_disk(rng: np.random.Generator, max_radius: float) -> np.ndarray:
    while True:
...
def actuator_hard_torque_limit(model: mujoco.MjModel) -> float:
    if model.nu <= 0:
        raise ValueError("model has no actuators")
    if model.actuator_ctrllimited.shape[0] != model.nu:
        raise ValueError("unexpected actuator control metadata")

    limits: list[float] = []
    for actuator_idx in range(model.nu):
        if not bool(model.actuator_ctrllimited[actuator_idx]):
            raise ValueError("all actuators must have finite ctrlrange for exploration dataset generation")
        lo, hi = model.actuator_ctrlrange[actuator_idx]
        limits.append(float(min(abs(lo), abs(hi))))
    return float(min(limits))


def generate_iid_uniform_torque(
    ...
    amplitude = float(min(abs(scale), hard_limit))
    return rng.uniform(-amplitude, amplitude, size=(num_steps, nu)).astype(np.float64)
...
def generate_smooth_torque(
    ...
    for dim in range(nu):
        for _ in range(num_sin):
            amp = rng.uniform(0.15 * amplitude, amplitude)
            freq = rng.uniform(0.4, 2.6)
            phase = rng.uniform(0.0, 2.0 * math.pi)
            torque[:, dim] += amp * np.sin(2.0 * math.pi * freq * t + phase)
    return np.clip(torque, -hard_limit, hard_limit)
...
def generate_torque_sequence(
    ...
    if torque_mode == "iid_uniform":
        return generate_iid_uniform_torque(
            rng=rng,
            num_steps=num_steps,
            nu=nu,
            scale=torque_scale,
    ...
    if torque_mode == "smooth_random":
        return generate_smooth_torque(
            rng=rng,
            num_steps=num_steps,
            dt=dt,
            nu=nu,
    ...
    raise ValueError(f"unsupported torque_mode={torque_mode}")
...
def init_worker_model(xml_path: str, dt: float) -> None:
    global _WORKER_MODEL, _WORKER_XML_PATH, _WORKER_DT, _WORKER_IDS, _WORKER_TORQUE_LIMIT
    ...
    _WORKER_TORQUE_LIMIT = actuator_hard_torque_limit(model)


def get_or_create_worker_model(xml_path: str, dt: float) -> tuple[mujoco.MjModel, dict[str, int], float]:
    global _WORKER_MODEL, _WORKER_XML_PATH, _WORKER_DT, _WORKER_IDS, _WORKER_TORQUE_LIMIT
    if _WORKER_MODEL is None or _WORKER_XML_PATH != xml_path or _WORKER_DT != float(dt):
        init_worker_model(xml_path, dt)
    return _WORKER_MODEL, _WORKER_IDS, float(_WORKER_TORQUE_LIMIT)
...
def set_reacher_state(
    ...
    data.qpos[ids["target_x_qpos"]] = marker_xy[0]
    data.qpos[ids["target_y_qpos"]] = marker_xy[1]
...
def simulate_dataset_rollout(
    ...
    set_reacher_state(model, data, start_arm_qpos, start_arm_qvel, marker_xy, ids)
    ...
    for step in range(num_steps):
        ...
        mom_curr = mass_matrix @ qvel_curr
        ...
        mujoco.mj_fullM(model, mass_matrix, data.qM)
        mom_next = mass_matrix @ data.qvel
...
def build_exploration_trajectory(task: tuple) -> tuple[bool, str, dict | None]:
    (
        xml_path,
        dt,
        trajectory_length,
    ...
    model, ids, hard_torque_limit = get_or_create_worker_model(xml_path, dt)

    source_xy = sample_xy_in_disk(rng, max_radius=source_radius)
    elbow_branch = 1 if rng.random() < 0.5 else -1
    start_arm_qpos = ik_2link(source_xy, elbow_branch=elbow_branch)
    start_arm_qvel = rng.uniform(-qvel_scale, qvel_scale, size=2)
    torque_seq = generate_torque_sequence(
        rng=rng,
    ...
    if np.any(np.abs(result["seq_torque"]) > hard_torque_limit + 1e-6):
        return False, "torque_limit", None
    ...
    result["start_qpos"] = start_arm_qpos.astype(np.float32)
    result["start_qvel"] = start_arm_qvel.astype(np.float32)
    result["source_xy"] = source_xy.astype(np.float32)
    result["elbow_branch"] = np.int32(elbow_branch)
    result["torque_mode"] = str(torque_mode)
...
def write_h5_header(
    ...
    file.attrs["generator"] = "exploration_reacher"
...
def write_trajectory_group(file: h5py.File, traj_index: int, result: dict) -> None:
    ...
    group.create_dataset("start_qpos", data=result["start_qpos"], dtype="f4")
    group.create_dataset("start_qvel", data=result["start_qvel"], dtype="f4")
    group.create_dataset("source_xy", data=result["source_xy"], dtype="f4")
    group.attrs["elbow_branch"] = int(result["elbow_branch"])
    group.attrs["torque_mode"] = result["torque_mode"]
...
def generate_split(
    ...
    reject_counts = {"qvel_limit": 0, "qacc_limit": 0, "non_finite": 0, "torque_limit": 0}
        ...
        try:
            while accepted < num_trajectories:
                ...
                if use_parallel:
                    ...
                    results = list(executor.map(build_exploration_trajectory, tasks, chunksize=chunksize))
                else:
                    results = [build_exploration_trajectory(task) for task in tasks]
    ...
    report = {
        ...
        "accept_rate": float(accepted / max(1, total_generated)),
        "reject_counts": {k: int(v) for k, v in sorted(reject_counts.items())},
        "config": config,
    }
    print(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate exploratory Reacher HDF5 datasets for unconditional DPF MVPs")
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/home/gsang/Projects/hnn_guided_dpf/data/reacher_exploration_dt0p001_len500_smooth_random",
    )
    parser.add_argument(
        "--xml_path",
        type=str,
    ...
    parser.add_argument("--train_trajectories", type=int, default=2000)
    parser.add_argument("--val_trajectories", type=int, default=200)
    parser.add_argument("--trajectory_length", type=int, default=500)
    parser.add_argument("--dt", type=float, default=0.001)
    parser.add_argument("--source_radius", type=float, default=0.18)
    parser.add_argument("--qvel_scale", type=float, default=0.8)
    parser.add_argument("--torque_mode", type=str, choices=["iid_uniform", "smooth_random"], default="smooth_random")
    ...
    if args.source_radius <= 0.0 or args.source_radius > REACH_MAX:
        raise ValueError(f"source_radius must be in (0, {REACH_MAX}]")

    output_dir = Path(args.output_dir)
    train_path = output_dir / f"train_traj_{args.train_trajectories}-steps_{args.trajectory_length}.h5"
    val_path = output_dir / f"val_traj_{args.val_trajectories}-steps_{args.trajectory_length}.h5"
    ...
    print(f"  source_radius: {args.source_radius}", flush=True)
    print(f"  qvel_scale: {args.qvel_scale}", flush=True)
    print(f"  torque_mode: {args.torque_mode}", flush=True)
    ...
    report = {"train_split": train_report}
    if args.val_trajectories > 0:
        val_report = generate_split(
