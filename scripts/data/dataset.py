...
def infer_hnn_torque_alignment(h5_attrs) -> str:
    ...
    New synchronized datasets explicitly store pre-step state plus interval-aligned
    torque/derivatives. Older datasets stored post-step state and require shifting
    torque by one sample for HNN transition supervision.
    """
    ...
    if isinstance(state_alignment, bytes):
        state_alignment = state_alignment.decode()
    if isinstance(torque_alignment, bytes):
        torque_alignment = torque_alignment.decode()
    if isinstance(derivative_alignment, bytes):
        derivative_alignment = derivative_alignment.decode()

    if (
        state_alignment == 'pre_step'
        and torque_alignment == 'interval_mean'
        and derivative_alignment == 'forward_difference'
    ):
        return "legacy"
...
class TrajectoryHNNCached(Dataset):
    ...
    def __init__(
        ...
        torque_alignment: str = "auto",
    ):
        super().__init__()
        print(f"Loading dataset into memory from {h5_path}...")
        if torque_alignment not in {"auto", "legacy", "transition_next"}:
            raise ValueError(
                f"Unknown torque_alignment={torque_alignment!r}; "
                "expected 'auto', 'legacy' or 'transition_next'"
            )
        self.torque_alignment = torque_alignment
        ...
        with h5py.File(h5_path, 'r') as f:
            ...
            self.dt = f.attrs['dt']
            self.resolved_torque_alignment = (
                infer_hnn_torque_alignment(f.attrs)
                if self.torque_alignment == "auto"
                else self.torque_alignment
            )
            ...
            for i in range(self.num_traj):
                ...
                qacc = traj['seq_qacc'][:]
                mom = traj['seq_mom'][:]
                mom_dot = traj['seq_mom_dot'][:]
                torque = traj['seq_torque'][:]

                if self.resolved_torque_alignment == "transition_next":
                    # Dataset generation records state after applying torque at the same
                    # index, so use tau[t+1] with state/derivatives at time t.
                    qpos = qpos[:-1]
                    qvel = qvel[:-1]
                    qacc = qacc[:-1]
                    mom = mom[:-1]
                ...
                all_qacc.append(qacc)
                all_mom.append(mom)
                all_mom_dot.append(mom_dot)
        ...
        print(
            f"HNN torque alignment request={self.torque_alignment}, "
            f"resolved={self.resolved_torque_alignment}"
        )
